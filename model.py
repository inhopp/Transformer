...
class TransformerBlock(nn.Module):
    def __init__(self, embed_size, heads, dropout, forward_expansion):
        ...
        self.norm1 = nn.LayerNorm(embed_size) # for self-attention
        self.norm2 = nn.LayerNorm(embed_size) # for feed-forward network
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for layer in self.layers:
            out = layer(out, out, out, mask) # In the Encoder the query, key, value are all the same
